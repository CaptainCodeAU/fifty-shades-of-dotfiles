...
default when something *outside your control* runs the command — a git hook, a cron
...
a `uv tool uninstall`. When a project *requires* one mode, declare it in the project:
...
| *(empty)* | Auto-detect based on context (recommended)               |
...
- **`settings/iterm2/profiles.json`**: Older, colors-only iTerm2 profile export for manual import (Profiles > Other Actions > Import JSON Profiles). Superseded by `settings/iterm2/prefs/` below.
