...
| `speak-clipboard`         | Speak the clipboard aloud with terminal furniture stripped (ANSI, PUA glyphs, rule runs) and typography mapped to ASCII, through one chunked `speak-render` process per press; toggle = same text stops, new text replaces (see `docs/HERDR.md`) | direct  |
