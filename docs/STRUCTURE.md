...
| `toolchain-stocktake`     | Read-only survey of the machine's EXISTING Python/Node toolchain (pyenv/conda/asdf/mise, pipx tools, npm globals, non-default registry) before install.sh's opinionated takeover lands                   | direct  |
| `project-impact-scan`     | Read-only scan of existing Python/Node PROJECTS for which npm/venv-based ones break once pnpm/uv are enforced, with a per-project migration hint and a durable report for a later Claude Code session    | direct  |
