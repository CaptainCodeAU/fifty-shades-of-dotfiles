...
| `herdr-cooldown-check`    | Enforce the 7-day release cooldown for herdr: version/age gate, brew pin, phone-home guards (see `docs/HERDR.md`)                                                                                                                                                                                              | direct  |
...
| `speak-clipboard`         | Speak the clipboard aloud with terminal furniture stripped (ANSI, PUA glyphs, rule runs) and typography mapped to ASCII, through one `say2` process per press (falls back to Simone, then plain `say`, if the premium voice is unavailable); toggle = same text stops, new text replaces (see `docs/HERDR.md`) | direct  |
...
| `HERDR.md`                         | herdr: the 7-day release cooldown, daemon persistence, speak-selection bindings, and the tmux comparison                                                                                      |
