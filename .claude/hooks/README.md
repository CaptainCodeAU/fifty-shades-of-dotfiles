...
Runs on `SessionStart`, read-only. Thin wrapper: locates the `ci-watch` engine and runs its session render for the welcome dashboard. Read-only against your repos, queries `gh` live and falls back to a per-target cache when offline. A red that stays red **escalates**, and it cannot be dismissed by being seen — only by CI going green or a deliberate `ci-watch --snooze`. **NOT CURRENTLY WIRED — this file does not run** (verified 2026-08-01: the string `ci-watch.sh` appears in no settings file). ci-watch itself IS live: `~/.claude/settings.json` calls the `~/.local/bin/ci-watch` engine inline, at USER level, so it runs in every project — look there if it seems to have stopped, not here. The wrapper is kept as the documented description of the hook's guarantees, and because the other three hooks in this directory follow this shape. Note its repo-source fallback (`$PROJECT_DIR/home/.local/bin/ci-watch`) only resolves INSIDE the dotfiles repo, so wiring it at user level would not buy the fallback anywhere else — which is why it stayed unwired. See [`docs/CI_WATCH.md`](../../docs/CI_WATCH.md).
