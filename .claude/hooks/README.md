...
| `enforce-builtin.sh`         | `PreToolUse`   | `Bash`            | project                                         | deny `builtin <non-builtin>` (shared scanner since 2026-09-23)                                                                                                               | `--selftest`                   |
...
| `validate-bash.sh`           | `PreToolUse`   | `Bash`            | manifest `project` (`home/.claude/hooks/`)      | deny `rm -rf /`, force push to main/master, bare `git reset --hard`, `git clean -fd`; the nested-Claude skip-permissions aliases, `gpf!`, `brew install\|reinstall\|upgrade` | `--selftest`                   |
...
home/.claude/hooks/                 stowed to ~/.claude/hooks/, registered by the manifest
  validate-bash.sh                  manifest project: destructive git and rm, nested-Claude aliases, gpf!, brew install
...
Runs on `PreToolUse` for `Bash`. Since 2026-09-23 a LEADING `cd DIR && rest` (or `;`, or a newline) is REWRITTEN to `(builtin cd DIR && rest` plus a newline and `)`, so the session's working directory cannot move; the `.zshrc` aliases that expand to `cd` (`..` `...` `....` `.....` `~`) are rewritten the same way, and the live aliases that change directory but cannot be rewritten safely (`-`, `1`..`9`, `grt`) are denied. Every other `cd` shape is still denied: use absolute paths, `git -C <path>`, or `builtin cd`. Since 2026-09-23 (Gavin's proposal E) a `cd` inside an explicit `( )` subshell, such as `(cd /x && make)`, is allowed untouched: it cannot outlive the subshell. A `cd` inside `{ }` is still denied, because a brace group runs in the current shell and its `cd` persists (measured in zsh 5.9). It uses the shared scanner in `home/.claude/hooks/conv-shscan.awk` (reached through this repo's path), which reads quotes, `$(...)`, heredocs and zsh operators properly; the old sed strip denied a quoted `)` inside `$(...)` and a multi-line `"..."` commit message. A command that also trips the uv or pnpm rule is denied with one combined message, because two rewriting hooks race (`docs/CLAUDE_HOOKS.md`, "Rewriting a tool call"). Project-level on purpose; see the note above and W-20260921-07 (shadow-mode traveller).

### enforce-builtin.sh

Runs on `PreToolUse` for `Bash`. Blocks `builtin` with non-builtins (`builtin git`, `builtin swift`), which zsh rejects anyway; allows real builtins (`cd`, `echo`, `printf`, `pushd`, `export`, and so on). Companion of `enforce-no-cd.sh`. Since 2026-09-23 (proposal F) it uses the same shared scanner (mode `builtin`), reached the same way: the old sed strip denied a quoted `)` inside `$(...)`, a multi-line `"..."` message and `builtin "echo"`, and missed a second `builtin` in one command and one inside `{ }`, `if` or `!`. `--selftest` has 44 arms, including every shape the sed version got right.
...
On 2026-09-23 (Gavin's proposals A, B, C) `validate-bash.sh` also denies, through the shared scanner in mode `guard`: the six `.zshrc` aliases that launch a nested Claude with `--dangerously-skip-permissions` (`ci cr ct cpr cd_ cskip`; `cb` does not skip permissions and is allowed), the oh-my-zsh `gpf!` (`git push --force`; the lease forms `gpf` and `gpsupf` stay allowed), and `brew install`, `instal`, `reinstall` and `upgrade` (ask Gavin; `list`, `info`, `search`, `outdated` stay allowed, and the `.zshrc` `brew()` guard is interactive-only). A hook sees the typed text, never the alias expansion, which is why the older rules never saw these names. The alias is denied at command position only, including inside `$(...)`, backticks and `eval`, where zsh expands aliases (measured). `conv-shscan.awk` and `conv-hooklib.sh` must be stowed beside it; without them the three rules fall back to a crude deny by name and say so.
...
.claude/hooks/enforce-builtin.sh --selftest
~/.claude/hooks/enforce-secret-probe.sh --selftest
mv-wrapper-selftest                                   # the .zshrc mv()/cp() wrappers
