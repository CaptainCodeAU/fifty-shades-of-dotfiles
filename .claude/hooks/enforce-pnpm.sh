...
if echo "$STRIPPED" | grep -qE '(^|[;&|]\s*)npm\s+'; then
  echo "BLOCKED: Use 'pnpm' instead of npm"
...
if echo "$STRIPPED" | grep -qE '(^|[;&|]\s*)yarn\s+'; then
  echo "BLOCKED: Use 'pnpm' instead of yarn"
...
if echo "$STRIPPED" | grep -qE '(^|[;&|]\s*)yarn\s*$'; then
  echo "BLOCKED: Use 'pnpm install' instead of yarn"
...
if echo "$STRIPPED" | grep -qE '(^|[;&|]\s*)npx\s+'; then
  echo "BLOCKED: Use 'pnpm dlx' instead of npx"
