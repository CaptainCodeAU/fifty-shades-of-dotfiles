...
- **ci-watch wiring** (engine + wrapper exist; is the dashboard actually wired?): !`R=$(git rev-parse --show-toplevel 2>/dev/null); grep -q 'ci-watch' "$R/.claude/settings.json" 2>/dev/null && echo "ci-watch: WIRED" || echo "ci-watch: DORMANT (.claude/hooks/ci-watch.sh exists, not in settings.json) - backlog item"`
...
6. **DATED-HISTORICAL feature stamps are load-bearing** - `NEW (v2.1.83+)`, `sandbox.credentials (v2.1.187)` record WHEN a feature landed; bumping them to the live host version destroys real history. Only an "as of installed vX" scope stamp gets bumped.
7. **Two memory stores** - reconcile the `~/.claude/...` per-project index; NEVER the repo `MEMORY/WORK/`.
