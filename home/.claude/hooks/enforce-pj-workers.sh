...
_conv_rc=$?
if [ "$_conv_rc" -ne 0 ] || ! declare -F conv_scan conv_deny conv_log >/dev/null 2>&1; then
...
EOF
  ...
  _lf top   deny "broken lib (error at the top), the trigger: deny by name" 'herdr agent start w --kind claude --pane w1:p1'
  _lf top   allow "broken lib (error at the top), no trigger: quiet" 'ls -la'
  _lf end   deny "broken lib (error at the end, functions defined), the trigger: deny" 'herdr agent start w --kind claude --pane w1:p1'
  _lf end   allow "broken lib (error at the end), no trigger: quiet" 'ls -la'
  _lf noere allow "broken lib, no ERE in the hook: quiet, not deny-all" 'herdr agent start w --kind claude --pane w1:p1'
  conv_selftest_end
fi
