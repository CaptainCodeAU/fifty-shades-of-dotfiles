...
Never start a Bash command with `cd` **in this repo** — [`enforce-no-cd.sh`](.claude/hooks/enforce-no-cd.sh) rewrites a LEADING `cd DIR && rest` into `(builtin cd DIR && rest)` so your working directory does not move, and denies every other `cd` shape (it tells you to use `git -C <path>`, an absolute path, or `builtin cd`), except a `cd` inside an explicit `( )` subshell, which cannot persist and is allowed since 2026-09-23. Since 2026-09-23 the uv and pnpm hooks also rewrite clear slips instead of denying them; `docs/CLAUDE_HOOKS.md` "Rewriting a tool call" has the rules. **This is THIS REPO'S OWN PreToolUse hook, registered in `.claude/settings.json`, NOT a built-in Claude Code guard** — a sentence that said "the harness hard-rejects" it stood here until 2026-09-21 and was false. P5.6 measured the last 20 transcripts of two other projects: **113 of 245 leading-`cd` commands in `win_go_app_test` and 10 of 51 in `Network_Plan` ran without error**, because the hook is registered nowhere but here. Ruled 2026-09-21: it stays project-only. Treat the denial as a signal to change the command _shape_ (reach for `git -C`/absolute paths), not to retry the same `cd`-prefixed command. A denied `cd` exits non-zero, so if it was batched with sibling calls it cancels all of them (see next paragraph) — which reads as a "stuck loop" but is really one repeated mistake.

The general form is worth more than the correction: **a rule you only ever meet inside one repo will be mistaken for a property of the tool.** Nothing in a denial message says which layer issued it, so "I cannot do X" and "I cannot do X _here_" look identical, and the wrong one got written down and believed for weeks.
...
- **`$GH_TOKEN` IS UNSET IN A CLAUDE SESSION, and this line said the opposite until 2026-09-22.** `__claude_launch` used to export a read-only fine-grained PAT as `$GH_TOKEN` in every session; it was **removed on 2026-09-18** (the reason is written into `~/.zsh_claude_launch` at the line where the injection used to be: every session started with a GitHub key in its environment, where any command could read it and any transcript could record it, and that is how tokens reached transcripts from June 2026). Every consumer now fetches its own credential and fails closed. Measured 2026-09-22 in a `pj` session: `GH_TOKEN` unset, `GITHUB_TOKEN` unset, control `HOME` set.
- **So `-H "Authorization: Bearer $GH_TOKEN"` sends an EMPTY bearer, and GitHub answers 401.** Three arms against the same URL: empty bearer **401**, no `Authorization` header at all **200**, a bogus non-empty token **401**. That 401 cost F8a a wrongly-diagnosed item (`W-20260922-A09`, filed and declined in the same session) because it reads exactly like an expired credential — and F5c read the same status the same wrong way, blaming a rotation when the cause was different again. **A 401 cannot tell an empty token from a bad one.** The instrument is `[ -n "${V-}" ] && echo "SET len=${#V}"`, never the status code. For public reads, send no header: unauthenticated works and is rate-limited, not refused.
